...
IF "%1" == "LIBBATCH" (
  setlocal ENABLEDELAYEDEXPANSION
    cd "!%1_BUILD_DIR!" && echo Starting Salome%1 in VC14 && start "Starting Project" %msvc_exe% LibBatch.sln
  endlocal
  goto END
)
...
IF "%1" == "MEDCOUPLING" (
  setlocal ENABLEDELAYEDEXPANSION
    cd "!%1_BUILD_DIR!" && echo Starting Salome%1 in VC14 && start "Starting Project" %msvc_exe% MEDCoupling.sln
  endlocal
  goto END
)

setlocal ENABLEDELAYEDEXPANSION
  cd "!%1_BUILD_DIR!" && echo Starting Salome%1 in VC14 && start "Starting Project" %msvc_exe% Salome%1.sln
endlocal
goto END
