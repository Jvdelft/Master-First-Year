...
all_modules.append('FIELDS')
all_modules.append('SMESH')
all_modules.append('SHAPER')
...
for module in modules:
    ...
    os.chdir("..\MODULES\BUILD\%s"%(module.upper()))
    # --
    m = module
    ...
    cmd += r'-G "Visual Studio 14 2015%s" '%(appendix)
    if module == "GUI":
    ...
    if module == "PARAVIS":
       cmd += r'-DSALOME_PARAVIS_USE_GEOM_SOURCE=ON -DSALOME_PARAVIS_USE_SMESH_SOURCE=ON '
    cmd += r'"%s"'%(src_dir)   
    print("%s"%(cmd))
    os.system(cmd)
    ...
    cmd1 = ""
    if module in ["MEDCOUPLING", "LIBBATCH", "SHAPER"]:
